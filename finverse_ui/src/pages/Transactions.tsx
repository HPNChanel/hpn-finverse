...
// ...existing code...
  const handleCreateTransaction = async (transactionData: CreateTransactionRequest) => {
    try {
      ...
      if (!transactionData.wallet_id || transactionData.wallet_id === 0) {
      ...
      if (transactionData.transaction_type === null || transactionData.transaction_type === undefined) {
      ...
      if (response.data) {
        ...
        // CRITICAL ADDITION: Refresh budget data after expense transaction
        if (transactionData.transaction_type === 1 && transactionData.category_id) {
          try {
            // Small delay to allow backend budget processing
            setTimeout(async () => {
              await api.post('/budgets/update-all-usage');
              console.log('Budget usage updated after transaction creation');
            }, 500);
          } catch (budgetError) {
            console.warn('Failed to update budget usage:', budgetError);
          }
        }
  ...
  const handleUpdateTransaction = async (id: number, transactionData: UpdateTransactionRequest) => {
    try {
      ...
      if (response.data) {
        ...
        // CRITICAL ADDITION: Refresh budget data after transaction update
        try {
          setTimeout(async () => {
            await api.post('/budgets/update-all-usage');
            console.log('Budget usage updated after transaction update');
          }, 500);
        } catch (budgetError) {
          console.warn('Failed to update budget usage:', budgetError);
        }
  ...
  const handleDeleteTransaction = async (id: number) => {
    try {
      ...
      // CRITICAL ADDITION: Refresh budget data after transaction deletion
      try {
        setTimeout(async () => {
          await api.post('/budgets/update-all-usage');
          console.log('Budget usage updated after transaction deletion');
        }, 500);
      } catch (budgetError) {
        console.warn('Failed to update budget usage:', budgetError);
      }
