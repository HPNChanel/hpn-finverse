...
api.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      ...
      console.log(`🔑 Authorization header added for ${config.method?.toUpperCase()} ${config.url}`);
    } else {
      console.warn(`⚠️ No access token found for ${config.method?.toUpperCase()} ${config.url}`);
    }
    return config;
  },
  (error) => {
